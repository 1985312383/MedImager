...
为确保应用具备良好的国际化能力，当前采用 YAML 源翻译文件编译为 JSON 运行时 catalog 的工作流。

1.  **代码实现**: 所有需要翻译的面向用户字符串都使用稳定 key，例如 `t("mainwindow.file_f")`。不要使用可见中文或其他自然语言文本作为 key。
2.  **维护翻译文件**: 人工维护 `medimager/i18n/locales/<language>.yml`。默认语言为 `en_US`，其他语言必须保持相同 key 集合。
3.  **编译运行时 catalog**: 在项目根目录运行 `python translation_tools/main.py`，它会生成 `medimager/i18n/compiled/<language>.json`。
4.  **运行时加载**: `medimager.utils.i18n.TranslationManager` 根据用户设置加载 JSON catalog，并通过 `t(...)` 提供翻译文本。
5.  **旧链路状态**: Qt `.ts/.qm`、`QTranslator` 兼容层和 `self.tr(...)` 源文本映射已经移除。
...
*   **关键逻辑**:
    *   `main()`:
        ...
        *   根据 `settings.py` 中的语言设置加载对应的 JSON catalog。
        *   实例化 `MainWindow`。
        *   显示主窗口并启动事件循环。
...
**🚧 规划中的功能**:
- DICOM 正确性与专业测试基线（CT/MR/CR/US/PET、压缩 DICOM、多 frame、非轴位、缺失标签、PixelSpacing 差异）
- 大序列加载和显示性能基准
- 完整的多视图同步套件
- 3D渲染和MPR重建
- DICOMDIR / 本地病例索引

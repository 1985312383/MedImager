...
- **单击序列**: 如果序列已绑定到视图，激活该视图；如果未绑定，分配到当前活动视图。
- **双击序列**: 在新的可用视图中打开序列，如果没有可用视图则替换当前活动视图。
- **右键菜单**:
  - "在新视图中打开"
  - "替换当前视图"
  - "移除序列"
...
│   ├── multi_series_manager.py # 【新增/增强】管理多个ImageDataModel实例，负责序列的加载、绑定和同步。
│   ├── dicom_parser.py       # 负责从文件解析DICOM数据。
│   ├── roi.py                # 定义各种ROI（圆形、矩形等）的几何形状和行为。
│   ├── analysis.py           # 提供ROI统计计算等分析功能。
│   └── sync_manager.py       # 【新增】管理多视图间的同步逻辑（窗位、切片、缩放等）。
│
├── ui/                       # 【视图/控制器层】所有UI组件
│   ├── main_window.py        # 【增强】实现MainWindow类，支持多序列多视图管理。
│   ├── multi_viewer_grid.py  # 【增强】实现MultiViewerGrid，支持动态布局和视图同步。
│   ├── image_viewer.py       # 【增强】实现ImageViewer，支持序列绑定和同步功能。
│   ├── main_toolbar.py       # 【增强】添加布局和同步工具按钮。
│   │
│   ├── panels/               # 可停靠的面板
│   │   ├── series_panel.py     # 【重新设计】支持多序列管理、分组显示、拖拽绑定。
│   │   ├── dicom_tag_panel.py  # 【增强】支持多序列信息切换显示。
│   │   ├── analysis_panel.py   # 【新增】显示分析结果汇总。
│   │   └── comparison_panel.py # 【新增】支持序列间的对比分析。
│   │
│   ├── tools/                # 交互工具
│   │   ├── base_tool.py        # 【增强】支持多视图环境下的工具行为。
│   │   ├── default_tool.py     # 【增强】支持多视图的ROI选择和同步操作。
│   │   ├── measurement_tool.py # 【增强】支持多视图环境下的测量工具。
│   │   └── roi_tool.py         # 【增强】支持多视图环境下的ROI绘制。
│   │
│   ├── widgets/              # 【新增】自定义控件
│   │   ├── series_list_widget.py    # 【新增】增强的序列列表控件。
│   │   ├── view_binding_widget.py   # 【新增】视图绑定管理控件。
│   │   ├── layout_selector.py       # 【新增】布局选择器控件。
│   │   └── cross_reference_overlay.py # 【新增】交叉参考线覆盖控件。
│   │
│   └── dialogs/              # 对话框窗口
│       ├── settings_dialog.py  # 【增强】添加多视图相关设置页面。
│       ├── custom_wl_dialog.py # 实现自定义窗位输入对话框。
│       └── series_info_dialog.py # 【新增】序列详细信息对话框。
│
└── utils/                    # 【通用工具】
    ├── logger.py             # 配置日志记录。
    ...
    ├── i18n.py               # 国际化相关工具。
    └── layout_manager.py     # 【新增】布局配置的保存和加载。
``` 
...
这个完善的设计文档为MedImager的多序列多视图功能提供了全面的架构指导。接下来我们可以根据这个设计开始具体的实现工作。

...
MedImager 是一款开源医学影像查看与分析工具，长期目标是逐步接近 RadiAnt 级阅片工作流。2.4 版本在可靠的 2D DICOM 基线上新增患者空间容积模型与轴位、冠状位、矢状位联动 MPR：多序列查看、测量与 ROI 分析、专业合成 DICOM 测试覆盖、标注持久化和可重复性能基准。
...
创建一款可逐步成长到 RadiAnt 级工作流的开源医学影像查看器。MedImager 2.4 已实现经过几何校验的正交 MPR；后续版本将在此基础上推进 DICOMDIR/PACS、挂片布局、斜位重建和更完整的临床式阅片工作流。

## 2. 核心功能

### ✅ V2.0 - 2D DICOM 基线版本 (可发布)
- [x] **文件处理**:
